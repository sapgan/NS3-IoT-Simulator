...
#ifndef IPV6_ADDRESS_HELPER_CUSTOM_H
#define IPV6_ADDRESS_HELPER_CUSTOM_H
...
/**
 * @brief A helper class to make life easier while doing simple Ipv6 address
 * assignment in scripts.
 *
 * This class is a very simple Ipv6 address generator.  You can think of it
 ...
 * assignment situation you may want to look at the Ipv6AddressGenerator which
 * does recognize that IP address and network number generation is part of a
 * global problem.  Ipv6AddressHelperCustom is a simple class to make simple problems
 * easy to handle.
 *
 * We do call into the global address generator to make sure that there are
 * no duplicate addresses generated.
 *
 * @see Ipv6AddressGenerator
 */
class Ipv6AddressHelperCustom
{
public:
/**
 * @brief Construct a helper class to make life easier while doing simple Ipv6
 * address assignment in scripts.
 */
  Ipv6AddressHelperCustom (bool checkAddressDuplication = true);
 ...
 */
  Ipv6AddressHelperCustom (Ipv6Address network, Ipv6Prefix prefix,
                           bool checkAddressDuplication = true, Ipv6Address base =  Ipv6Address("::1"));
 ...
 *
 * @param network The Ipv6Address containing the initial network number to
 * use during allocation.  The bits outside the network mask are not used.
 * @param mask The Ipv6Mask containing one bits in each bit position of the
 * network number.
 * @param base An optional Ipv6Address containing the initial address used for
 ...
 * looks up the Ipv6 interface corresponding to the net device.  It then sets
 * the Ipv6Address and mask in the interface to the appropriate values.  If
 ...
 * are asked to assign Ipv6 addresses to.
 *
...
private:
  ...
  bool     m_checkAddressDuplication; //!< disable or enable checking of address duplication (Ipv6AddressGenerator), default value = true
};

} // namespace ns3

#endif /* IPV6_ADDRESS_HELPER_CUSTOM_H */
