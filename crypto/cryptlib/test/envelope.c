...
static int testCMSDebugCheck( const TEST_TYPE testType, const FAULT_TYPE faultType )
	{
	int status;
...
static int testPGPDebugCheck( const TEST_TYPE testType, const FAULT_TYPE faultType )
	{
	int status;
...
static int envelopePGPDebugCheckAuth( const FAULT_TYPE faultType )
	{
	cryptSetFaultType( faultType );
	if( envelopeAuthent( ENVELOPE_TESTDATA, ENVELOPE_TESTDATA_SIZE, CRYPT_FORMAT_PGP, TRUE, TRUE, FALSE, 0 ) )
		{
