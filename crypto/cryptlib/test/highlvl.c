...
static int signData( const char *algoName, const CRYPT_ALGO_TYPE algorithm,
	...
	status = cryptCheckSignature( buffer, length, checkContext, hashCheckContext );
	if( cryptStatusError( status ) )
		{
...
static int conventionalExportImport( const CRYPT_CONTEXT cryptContext,
	...
	if( useAltAlgo && cryptObjectInfo.hashAlgo != altPrfAlgo )
		{
		printf( "Key wrap with PRF algorithm %d reported algorithm as %d,\n"
				"line %d.\n", altPrfAlgo, cryptObjectInfo.hashAlgo, 
				__LINE__ );
		free( buffer );
		return( FALSE );
		}
...
static int testDebugCheck( const TEST_TYPE testType, const FAULT_TYPE faultType )
	{
	int status;
