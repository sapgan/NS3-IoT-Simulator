...
/* State storage for JSON-formatted output */

typedef struct {
	/* The buffer used to write JSON data to */
	char *jsonBuffer;
	int jsonBufPos, jsonBufSize;
	
	/* State information to control formatting of JSON data.  addedItem
	   indicates that there's an item preceding this one that we have to
	   delimit with a comma before we output the current item */
	BOOLEAN addedItem;
	} JSON_INFO;

static JSON_INFO jsonInfo;
...
BOOLEAN jsonAdd( IN_ENUM( JSON ) const JSON_TYPE type, 
	...
	{
	char *jsonBufPtr = jsonInfo.jsonBuffer + jsonInfo.jsonBufPos;
	int jsonBufSize = jsonInfo.jsonBufSize - jsonInfo.jsonBufPos, length;

	REQUIRES_B( type > JSON_NONE && type < JSON_LAST );
	REQUIRES_B( ( ( type == JSON_BEGIN || \
					type == JSON_OBJECT_START || \
					type == JSON_OBJECT_END || \
					type == JSON_ARRAY_END ) && label == NULL ) || \
				( type != JSON_OBJECT_END && label != NULL ) );
	REQUIRES_B( ( type == JSON_BOOLEAN && \
				...
				( type == JSON_STRING && \
				  value == 0 && string != NULL ) || \
				( type == JSON_STRING_HEX && \
				  ( value >= 0 && value <= MAX_INTLENGTH ) && \
				  string != NULL ) || \
				( ( type == JSON_OBJECT_START ) || ( type == JSON_ARRAY_START ) && \
				  value == 0 && string == NULL ) || \
				( ( type == type == JSON_BEGIN  ) || ( type == JSON_OBJECT_END ) || \
				  ( type == JSON_ARRAY_END ) && \
				  value == 0 && string == NULL ) || \
	...
	/* Sanity check to make sure that the JSON information has been 
	   initialised */
	if( jsonInfo.jsonBuffer == NULL || \
		jsonInfo.jsonBufSize <= 4096 || \
		jsonInfo.jsonBufSize > MAX_INTLENGTH || \
		jsonInfo.jsonBufPos < 0 || \
		jsonInfo.jsonBufPos > jsonInfo.jsonBufSize )
		return( CRYPT_ERROR_NOTINITED );

	/* If there's a preceding item, add a comma to separate it from previous 
	   items */
	if( type != JSON_OBJECT_END && type != JSON_ARRAY_END && \
		jsonInfo.addedItem )
		{
		length = sprintf_s( jsonBufPtr, jsonBufSize, ", " );
		if( length <= 0 )
			return( FALSE );
		jsonBufPtr += length;
		jsonBufSize -= length;
		jsonInfo.jsonBufPos += length;
		}

	/* Output the given item in JSON format */
	switch( type )
		{
		case JSON_BEGIN:
			length = sprintf_s( jsonBufPtr, jsonBufSize, "[ " );
		...
		case JSON_BOOLEAN:
			length = sprintf_s( jsonBufPtr, jsonBufSize, "\"%s\" : %s", 
								label, value ? "true" : "false" );
			break;

		case JSON_NUMERIC:
			length = sprintf_s( jsonBufPtr, jsonBufSize, "\"%s\" : %d", 
								label, value );
			break;
		...
		case JSON_STRING_HEX:
			{
			int totalLength, i;

			length = sprintf_s( jsonBufPtr, jsonBufSize, "\"%s\" : \"0x", 
								label );
			if( length <= 0 )
				break;
			jsonBufPtr += length;
			jsonBufSize -= length + 1;	/* For final " */
			totalLength = length + 1;
			for( i = 0; i < value; i++ )
				{
				length = sprintf_s( jsonBufPtr, jsonBufSize, "%02X", 
									byteToInt( string[ i ] ) );
				if( length <= 0 )
					break;
				jsonBufPtr += length;
		...
		case JSON_ARRAY_START:
			length = sprintf_s( jsonBufPtr, jsonBufSize, "\"%s\" : [ ", 
								label );
			break;

		case JSON_ARRAY_ITEM:
			if( value + 2 >= jsonBufSize )
				{
				length = -1;
				break;
				}
			jsonBufPtr[ 0 ] = jsonBufPtr[ value + 1 ] = '\"';
			memcpy( jsonBufPtr + 1, label, value );
			length = value + 2;
			break;
	...
	if( length <= 0 )
		return( FALSE );
	jsonInfo.jsonBufPos += length;
	if( ( type == JSON_BEGIN ) || ( type == JSON_OBJECT_START ) || \
		( type == JSON_ARRAY_START ) )
		{
		/* If we've just started a composite object then there's no 
		   item present that needs to be delimited when the next item is 
		   added */
		jsonInfo.addedItem = FALSE;
		}
	else
		jsonInfo.addedItem = TRUE;

	return( TRUE );
	}

/* Set a data buffer to record keyset info in JSON format */

C_RET cryptSetJSONInfo( void *data, C_IN int dataLength )
	{
	assert( isReadPtr( data, dataLength ) );

	/* Make sure the input parameters are approximately valid */
	if( data == NULL )
		return( CRYPT_ERROR_PARAM1 );
	if( dataLength < 4096 || dataLength > 0xFFFFFFL )
		return( CRYPT_ERROR_PARAM2 );

	/* Initialise the JSON processing information */
	memset( data, 0, dataLength );
	memset( &jsonInfo, 0, sizeof( JSON_INFO ) );
	jsonInfo.jsonBuffer = data;
	jsonInfo.jsonBufSize = dataLength;
