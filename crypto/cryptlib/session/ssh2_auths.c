...
static int processUserAuth( INOUT SESSION_INFO *sessionInfoPtr,
	...
	*userAuthInfo = USERAUTH_ERROR;
	...
	for( i = 0; methodInfoTblPtr[ i ].type != METHOD_NONE && \
	...
	switch( methodInfoPtr->type )
		...
		case METHOD_PUBKEY:
			...
			/* Skip the first of the three copies of the algorithm name (see 
			   the comment in ssh2_cli.c for more on this).  We don't do 
			   anything with it because we're about to get two more copies 
			   of the same thing, and the key and signature information take 
			   precedence over anything that we find here */
	...
	if( methodInfoPtr->type == METHOD_PUBKEY )
		...
		int packetDataLength, sigDataLength;
		...
		/* Make sure that this key is valid for authentication purposes, in
		   ...
		   Since we've already got a copy of the key, we only do a presence 
		   check rather than actually fetching the key/certificate. */
		...
		if( cryptStatusError( status ) )
			...
			retExt( CRYPT_ERROR_INVALID,
					( CRYPT_ERROR_INVALID, SESSION_ERRINFO, 
					  "Client pubkey is not trusted for "
		...
		if( cryptStatusOK( status ) )
			...
			/* readAuthPacketSSH2() has stripped the ID byte from the start 
			   of the packet so we need to hash it in explicitly before we 
			   hash the rest of the packet */
		...
		/* Check the signature.  The reason for the min() part of the 
		   expression is that iCryptCheckSignature() gets suspicious of very 
		   large buffer sizes, for example if the client were to specify the 
		   use of a huge signature packet */
		status = sMemGetDataBlockRemaining( &stream, &sigDataPtr, 
											&sigDataLength );
		if( cryptStatusOK( status ) )
			{
			status = iCryptCheckSignature( sigDataPtr, sigDataLength, 
							CRYPT_IFORMAT_SSH, sessionInfoPtr->iKeyexAuthContext, 
							createInfo.cryptHandle, CRYPT_UNUSED, NULL );
			}
		krnlSendNotifier( createInfo.cryptHandle, IMESSAGE_DECREFCOUNT );
		if( cryptStatusError( status ) )
			{
