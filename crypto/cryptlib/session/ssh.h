...
#define SSH_MSG_KEX_INIT		30	/* DH, phase 1 */
#define SSH_MSG_KEX_REPLY		31	/* DH, phase 2 */
...
/* SSH has multiple ways to specify the same keyex algorithm, either
   ...
   a hash algorithm other than the usual SHA1/SHA2 in order to know that we 
   have to send the peer the group14 form of the suite name and not the 
   group-exchange form.  This requires that we recognise the special-case 
   algorithm indicator and fix it up later to SHA1 */

#define isPseudoHash( algorithm ) \
		( ( algorithm ) == CRYPT_ALGO_MD5 )
#define pseudoHashToHash( algorithm )	CRYPT_ALGO_SHA1
