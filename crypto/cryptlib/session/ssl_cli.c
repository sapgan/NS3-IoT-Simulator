...
static int readServerKeyex( INOUT STREAM *stream, 
							...
							const BOOLEAN isECC, const BOOLEAN isTLSLTS )
	{
	void *keyData;
	...
	*dhContextPtr = CRYPT_ERROR;
	...
	if( cryptStatusOK( status ) )
		...
		status = initDHcontextSSL( dhContextPtr, keyData, keyDataLength, 
						CRYPT_UNUSED, isECC ? CRYPT_ECCCURVE_P256 /* Dummy */ : \
...
static int beginClientHandshake( INOUT SESSION_INFO *sessionInfoPtr,
	...
	/* Build the client hello packet:
	  ...
	  [	uint16	extListLen			-- RFC 3546/RFC 4366
	   ...
	   before we know whether the server can handle them, and many older 
	...
	if( status == OK_SPECIAL )
		...
		/* The server has provided a session ID, if we sent one in our 
		   request then this is a resumed session */
		if( sessionIDsent )
			resumeSession = TRUE;
		status = CRYPT_OK;
		}
	...
	if( resumeSession )
		...
		if( handshakeInfo->sessionIDlength != sentSessionIDlength || \
			memcmp( handshakeInfo->sessionID, sentSessionID,
					sentSessionIDlength ) )
			{
			/* Something very odd is going on, the server has told us to 
			   resume a session that we didn't ask for or don't know about */
			retExt( CRYPT_ERROR_INVALID,
