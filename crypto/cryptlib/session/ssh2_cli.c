...
static int beginClientHandshake( INOUT SESSION_INFO *sessionInfoPtr,
	...
	/* SSH has multiple ways to specify the same keyex algorithm (see the 
	   long comment in ssh.h), if we're using one of the alternate 
	   mechanisms, signalled through the use of a pseudo-algorithm for the
	   exchange hash algorithm, we need to set it to the actual hash 
	   algorithm and also reset the DH static key if necessary */
	if( isPseudoHash( handshakeInfo->exchangeHashAlgo ) )
		{
		handshakeInfo->exchangeHashAlgo = \
						pseudoHashToHash( handshakeInfo->exchangeHashAlgo );
		if( handshakeInfo->serverKeySize != bitsToBytes( 2048 ) )
			{
			/* Destroy the existing static DH key, load the new one, and re-
			   perform phase 1 of the DH key agreement process */
			krnlSendNotifier( handshakeInfo->iServerCryptContext,
							  IMESSAGE_DECREFCOUNT );
			handshakeInfo->iServerCryptContext = CRYPT_ERROR;
	...
	if( handshakeInfo->requestedServerKeySize > 0 )
		...
		status = continuePacketStreamSSH( &stream, SSH_MSG_KEX_INIT,
										  &packetOffset );
		}
	if( cryptStatusOK( status ) )
		{
	...
	status = length = \
		readHSPacketSSH2( sessionInfoPtr,
						  ( handshakeInfo->requestedServerKeySize > 0 ) ? \
							SSH_MSG_KEX_DH_GEX_REPLY : SSH_MSG_KEX_REPLY,
