...
static int generateDLPPublicValues( INOUT PKC_INFO *pkcInfo, 
	...
	for( i = 0, iterationCount = 0; 
		 i < nFactors && iterationCount < FAILSAFE_ITERATIONS_LARGE; 
		 i++, iterationCount++ )
		{
	...
	for( primeFound = FALSE, iterationCount = 0;
		...
		for( i = nFactors - 2, innerIterationCount = 0; 
			 i >= 0 && innerIterationCount < FAILSAFE_ITERATIONS_LARGE; 
			 i--, innerIterationCount++ )
			{
		...
		for( innerIterationCount = 0; 
			 ...
			 innerIterationCount++ )
			{
			int innerInnerIterationCount;

			/* Assemble a new candidate prime 2 * q * primes + 1 from the 
			   currently indexed random primes */
			for( i = indexMoved, innerInnerIterationCount = 0; 
				 bnStatusOK( bnStatus ) && i >= 0 && \
					innerInnerIterationCount < FAILSAFE_ITERATIONS_LARGE; 
				 i--, innerInnerIterationCount++ )
				{
				CK( BN_mul( &llProducts[ i ], &llProducts[ i + 1 ],
							&llPrimes[ indices[ i ] ], &pkcInfo->bnCTX ) );
				}
			ENSURES( innerInnerIterationCount < FAILSAFE_ITERATIONS_LARGE );
			...
			for( i = 0, innerInnerIterationCount = 0; 
				 i < nFactors && \
					innerInnerIterationCount < FAILSAFE_ITERATIONS_LARGE; 
				 i++, innerInnerIterationCount++ )
				{
			...
			for( i = indexMoved - 1, innerInnerIterationCount = 0; 
				 i >= 0 && \
					innerInnerIterationCount < FAILSAFE_ITERATIONS_LARGE; 
				 i--, innerInnerIterationCount++ )
				{
