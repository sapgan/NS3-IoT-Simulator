...
BOOLEAN sanityCheckBignum( const BIGNUM *bignum )
	...
	if( !( bignum->flags & BN_FLG_SCRATCH ) )
		{
		for( i = bignum->top; i < bignum->dmax; i++ )
			assert( bignum->d[ i ] == 0 );
		}
#endif /* Debug mode */
...
BOOLEAN BN_set_bit( INOUT BIGNUM *bignum, 
	...
	if( bignum->top < wordIndex + 1 )
		...
		for( index = bignum->top, iterationCount = 0; 
			 index < wordIndex + 1 && iterationCount < iterationBound; 
...
BOOLEAN BN_clear_top( INOUT BIGNUM *bignum, 
	...
	for( i = bignum->top, iterationCount = 0; 
		 i < oldTop && iterationCount < iterationBound; 
		 i++, iterationCount++ )
		{
		bignum->d[ i ] = 0;
		}
...
BOOLEAN BN_from_montgomery( INOUT BIGNUM *ret, INOUT BIGNUM *aTmp,
	...
	int bnStatus = BN_STATUS, i, iterationCount;
	...
	for( i = 0, iterationCount = 0; 
		 i < nLen && iterationCount < iterationBound; 
		 i++, iterationCount++ )
		{
